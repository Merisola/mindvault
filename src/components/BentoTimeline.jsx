...
import { generateAbstractCanvasVibe } from "@utils/mediaEngine";
...
const TAXONOMY_MAP = {
  moment: {
    ...
    text: "text-moment",
    activeClass: "bg-moment text-background border-moment",
  },
  vibe: {
    label: "Vibe",
    emoji: "💭",
    ...
    text: "text-vibe",
    activeClass: "bg-vibe text-background border-vibe",
  },
  spark: {
    label: "Spark",
    emoji: "💡",
    ...
    text: "text-spark",
    activeClass: "bg-spark text-background border-spark",
  },
  reminder: {
    label: "Reminder",
    emoji: "🔔",
    ...
    text: "text-reminder",
    activeClass: "bg-reminder text-background border-reminder",
  },
};
...
const FILTER_TABS = [
  {
    ...
    label: "All Nodes",
    emoji: "🌐",
    activeClass: "bg-accentCustom text-background border-accentCustom",
  },
  ...Object.entries(TAXONOMY_MAP).map(([id, value]) => ({ id, ...value })),
];
...
export default function BentoTimeline() {
  ...
  // Live Compute Filtering
  const filteredEntries = entries.filter((entry) => {
    const matchesCategory =
      activeFilter === "all" || entry.category === activeFilter;
    const textToSearch =
      `${entry.text_content} ${entry.auto_title}`.toLowerCase();
  ...
  return (
    <section className="mt-4 pb-12">
      {/* Search and Navigation Capsule Bar */}
      <div className="flex flex-col gap-4 mb-6">
        <div className="relative">
          <input
            ...
            placeholder="Search raw context keys or filenames (e.g. 'MOMENT_2026')..."
      ...
      {filteredEntries.length === 0 ? (
        <div className="text-center py-12 border border-dashed border-borderCustom rounded-2xl bg-surface/30">
          <p className="text-textSecondary text-sm italic">
            No matching mental records identified.
          </p>
          <p className="text-xs text-textSecondary/50 mt-1">
            Refine your search tokens or switch filters.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredEntries.map((entry) => {
            ...
            return (
              <div
                key={entry.id}
                className={`relative flex flex-col justify-between p-5 border ${meta.border} rounded-2xl transition-all duration-300 hover:translate-y-[-2px] hover:shadow-lg ${
                  entry.image_data
                    ? "bg-surface"
                    : `${generateAbstractCanvasVibe(entry.auto_title || entry.id)}`
                }`}
              >
                ...
                <div className="flex items-center justify-between gap-2 mb-4">
                  ...
                  <span className="text-xs font-medium text-textSecondary bg-background/60 backdrop-blur-sm px-2.5 py-1 rounded-full border border-borderCustom/40">
                ...
                {/* Checklist Action Items Footer */}
                {isReminder && (
                  <div className="mt-4 pt-3 border-t border-borderCustom/40 flex items-center justify-between">
